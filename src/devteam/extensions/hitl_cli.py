...
class HumanInTheLoop(CrewExtension):
    ...
    async def on_pause(self, thread_id: str, current_state: dict, next_node: str) -> dict | None:
        ...
        if current_state.get('clarification_question'):
            return await self._handle_clarification(current_state)
    ...
    async def _handle_clarification(self, current_state: dict) -> dict:
        ...
        user_input = await asyncio.to_thread(input, "Your Answer (or type 'exit' to abort): ")
    ...
    async def _handle_plan_approval(self, current_state: dict) -> dict:
        pending_tasks = current_state.get('pending_tasks', [])
        if not pending_tasks:
            return await self._handle_spec_approval(current_state)
        return await self._handle_task_plan_approval(current_state)

    async def _handle_spec_approval(self, current_state: dict) -> dict:
    ...
    async def _handle_task_plan_approval(self, current_state: dict) -> dict:
        specs = current_state.get('specs', 'No specifications available.')
        pending_tasks = current_state.get('pending_tasks', [])
        print("\n" + "="*60)
